...
df_test = []
for i in range(81):
    file_path = '../STUDY/DACON/data/test/%d.csv'%i
...
# 모델을 48번 돌릴것이기 때문에 트레인x 를 (48, 훈련수, 일수, 7) 로 잘라줄 것이다~!
x,y1,y2 = [],[],[]
for i in range(48):
...
x = np.array(x) # (48, 훈련수, 일수, 7)
y1 = np.array(y1) # (48, 훈련수, 1)
y2 = np.array(y2) # (48, 훈련수, 1)

print(x.shape)  # (48, 1087, 7, 7)
...
def mymodel():
    model = Sequential()
    model.add(Conv1D(filters=256, kernel_size=2, padding='same', input_shape=(7, 7)))
...
for i in range(48):
    ...
    for j in quantiles:
        model = mymodel()
        filepath_cp = f'../study/dacon/data/modelcheckpoint/dacon_09_{i:2d}_y1seq_{j:.1f}.hdf5'
    ...
    for j in quantiles:
        model = mymodel()
        filepath_cp = f'../study/dacon/data/modelcheckpoint/dacon_09_{i:2d}_y2seq_{j:.1f}.hdf5'
        cp = ModelCheckpoint(filepath_cp,save_best_only=True,monitor = 'val_loss')
        model.compile(loss = lambda y_true,y_pred: quantile_loss(j,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(j,y,y_pred)])
