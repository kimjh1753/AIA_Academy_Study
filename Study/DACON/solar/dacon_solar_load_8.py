...
def hahaha(a, x_train, y_train, x_val, y_val, x_test):
    x = []
    for q in quantiles:
        filepath_cp = f'../study/dacon/data/modelcheckpoint/solar_checkpoint6_time{i}-{a}-{q}.hdf5'
...
es = EarlyStopping(monitor = 'val_loss', patience = 21)
lr = ReduceLROnPlateau(monitor = 'val_loss', patience = 7, factor = 0.5, verbose = 1)

x_test_data = []
for j in range(81):
    file_path = '../study/dacon/data/test/' + str(j) + '.csv'
...
for i in range(48):
    ...
    x_test = x_test.reshape(567,6)
    x_test = scaler.transform(x_test)
    x_test = x_test.reshape(81,7,6)
...
sub.to_csv('../study/dacon/data/train/submission_0126_8.csv', index=False)