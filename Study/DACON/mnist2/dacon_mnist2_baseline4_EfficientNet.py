...
#     model = nn.DataParallel(model)
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy)>valid_best_accuracy:
            torch.save(model.state_dict(), '../study/dacon/data-2/model/EfficientNetB0-fold{}.pt'.format(fold))
...
with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('../study/dacon/data-2/model/EfficientNetB0-fold{}.pt'.format(fold)))
        model.eval()
...
submission.to_csv('../study/dacon/data-2/EfficientNetB0-fold0.csv', index=False)
