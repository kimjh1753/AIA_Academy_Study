...
labels_df = pd.read_csv('../study/dacon/data-2/dirty_mnist_2nd_answer.csv')[:]
imgs_dir = np.array(sorted(glob.glob('../study/dacon/data-2/dirty_mnist_2nd/*')))[:]
labels = np.array(labels_df.values[:,1:])

test_imgs_dir = np.array(sorted(glob.glob('../study/dacon/data-2/test_dirty_mnist_2nd/*')))
...
class EfficientNet_MultiLabel(nn.Module):
    def __init__(self, in_channels):
        ...
        self.network = EfficientNet.from_pretrained('efficientnet-b1', in_channels=in_channels)
...
#     model = nn.DataParallel(model)
    ...
    epochs=30
    batch_size=30         # 자신의 VRAM에 맞게 조절해야 OOM을 피할 수 있습니다.
    ...
    scheduler_poly_lr_decay = PolynomialLRDecay(optimizer, max_decay_steps=decay_steps, end_learning_rate=1e-7, power=0.9)
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy)>valid_best_accuracy:
            torch.save(model.state_dict(), '../study/dacon/data-2/model/EfficientNetB1-fold{}.pt'.format(fold))
...
submission = pd.read_csv('../study/dacon/data-2/sample_submission.csv')

with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('../study/dacon/data-2/model/EfficientNetB1-fold{}.pt'.format(fold)))
        model.eval()
...
submission.to_csv('../study/dacon/data-2/EfficientNetB1-fold1.csv', index=False)
